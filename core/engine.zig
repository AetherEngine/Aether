...
const logger = @import("platform").logging;
...
pub const Engine = struct {
    ...
    pub fn init(
        ...
        self.input.init_platform(self.allocator(.game), self.io) catch |err| switch (err) {
            error.OutOfMemory => return error.InputInitOutOfMemory,
            else => return err,
        };
        errdefer self.input.deinit_platform();
    ...
    fn step_frame_internal(self: *Engine, allow_sleep: bool) !void {
        ...
        while (self.run_loop.update_accum >= update_us) {
            ...
            Platform.input.update(self.input.event_sink(), self.input.effective_cursor_mode());
            self.input.update();
