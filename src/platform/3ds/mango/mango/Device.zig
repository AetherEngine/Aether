...
pub const Handle = enum(u32) {
    ...
    pub fn release(device: Handle) !ScreenCapture {
        const b_device: *Device = @ptrFromInt(@intFromEnum(device));
...
pub const VTable = struct {
    ...
    release: *const fn (dev: *Device) ReleaseDeviceError!ScreenCapture,
...
pub fn release(device: *Device) !ScreenCapture {
    ...
    return switch (create_info.code_type) {
        .psh => {
            ...
            const entry = blk: {
                ...
                while (it.next() catch return error.ValidationFailed) |entry| if (std.mem.eql(u8, entry.name, entrypoint_name)) {
                    break :blk entry;
                };
