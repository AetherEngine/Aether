...
pub fn setup(_: std.mem.Allocator, _: std.Io, input: input_api.EventSink) void {
    active_input = input;
    initialized = false;
...
pub fn begin_text_input_session(input: input_api.EventSink, request: *const input_api.TextInputRequest) input_api.TextSessionError!void {
    ...
    const initial_len = copy_initial_text(request.initial, &initial_buf);
    const initial = initial_buf[0..initial_len :0];

    var target_buf: [128:0]u8 = @splat(0);
    const target_text = copy_z(&target_buf, request.prompt);

    if (c.swkbdCreate(config, 0) != 0) {
        input.complete_text(initial_buf[0..initial_len], .cancelled);
        return;
    }
    ...
    const out_size = output_buffer_size(request.max_bytes);
    if (c.swkbdShow(config, out_buf[0..].ptr, out_size) == 0) {
        const len = bounded_z_len(out_buf[0..out_size]);
        input.complete_text(out_buf[0..len], .submitted);
    } else {
        input.complete_text(initial_buf[0..initial_len], .cancelled);
    }
}
...
fn diff_buttons(input: input_api.EventSink, buttons: u64) void {
    const Pair = struct { mask: u64, button: input_api.Button };
    const map = [_]Pair{
...
fn pump_axes(input: input_api.EventSink, buttons: u64) void {
...
fn pump_touch(input: input_api.EventSink) void {
...
fn copy_initial_text(text: []const u8, dst: []u8) usize {
