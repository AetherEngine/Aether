...
pub fn info() Info {
    var result: Info = switch (options.config.platform) {
        .psp => @import("psp/system.zig").info(),
        .nintendo_3ds => @import("3ds/system.zig").info(),
...
test "headless capability queries describe implemented input" {
    if (options.config.gfx == .headless) {
        try @import("std").testing.expect(!info().input.pointer);
        try @import("std").testing.expect(!info().input.native_text_entry);
    }
}
