...
export fn aether_input_key(key_code: u32, pressed: bool, repeat: bool, mods_bits: u32) void {
    ...
    const key: input_api.Key = std.enums.fromInt(input_api.Key, key_code) orelse return;
...
export fn aether_input_mouse_button(button_code: u32, pressed: bool, x: f32, y: f32) void {
    const input = active_input orelse return;
    if (button_code > @intFromEnum(input_api.MouseButton.Middle)) return;
    const button: input_api.MouseButton = @enumFromInt(button_code);
...
export fn aether_input_gamepad_button(button_code: u32, pressed: bool) void {
    const input = active_input orelse return;
    const button: input_api.Button = std.enums.fromInt(input_api.Button, button_code) orelse return;
...
export fn aether_input_gamepad_axis(axis_code: u32, value: f32) void {
    const input = active_input orelse return;
    const axis: input_api.Axis = std.enums.fromInt(input_api.Axis, axis_code) orelse return;
    input.deliver_gamepad_axis(axis, value);
}

fn decode_mods(bits: u32) input_api.ModifierSet {
    var mods: input_api.ModifierSet = .{};
