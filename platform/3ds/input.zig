...
var prev_touch_pos: input_api.Vec2 = .{};
var current_cursor_mode: input_api.CursorMode = .visible;
var active_input: ?input_api.EventSink = null;

pub fn setup(alloc: std.mem.Allocator, _: std.Io, input: input_api.EventSink) void {
    input_alloc = alloc;
    active_input = input;
    reset_state();
}
...
pub fn apply_cursor_mode(mode: input_api.CursorMode) void {
    current_cursor_mode = mode;
}

pub fn begin_text_input_session(input: input_api.EventSink, request: *const input_api.TextInputRequest) input_api.TextSessionError!void {
    ...
    const initial = copy_initial_text_for_osk(request, &initial_buf);

    var buttons = [_]SoftwareKeyboard.Config.Button{
        .button(.utf8("Cancel"), .none),
        .button(.utf8("OK"), .submits),
    };
    const hint = valid_utf8_prefix(request.prompt, SoftwareKeyboard.State.max_hint_text_len - 1);

    const osk_alloc = app.base.gpa;
    var swkbd = SoftwareKeyboard.normal(.{
        .max_length = osk_max_length(request.max_bytes),
        .buttons = &buttons,
        .initial_text = .utf8(initial),
        .hint = if (hint.len == 0) .default else .utf8(hint),
        .features = .{ .multiline = request.multiline },
        .dictionary = &.{},
    ...
    switch (result) {
        .right => {
            ...
            const text = clamp_utf8_to_max_bytes(out_buf[0..len], request.max_bytes);
            input.complete_text(text, .submitted);
        },
        else => input.complete_text(initial, .cancelled),
    }
}
...
fn pump_buttons(input: input_api.EventSink, entry: Hid.Pad.Entry) void {
...
fn pump_left_stick(input: input_api.EventSink, entry: Hid.Pad.Entry) void {
...
fn pump_ir(input: input_api.EventSink, pad: Hid.Pad.Entry) void {
...
fn pump_touch(input: input_api.EventSink, touch: Hid.Touch.State) void {
    ...
    if (touch.pressed) {
        const pos: input_api.Vec2 = .{
            .x = @floatFromInt(touch.x),
            .y = @floatFromInt(touch.y),
        };
        const delta: input_api.Vec2 = if (prev_touch_down)
...
fn release_touch_if_down(input: input_api.EventSink) void {
...
fn diff_button(input: input_api.EventSink, current: bool, previous: bool, button: input_api.Button) void {
...
fn copy_initial_text_for_osk(request: *const input_api.TextInputRequest, dst: []u8) []const u8 {
    const limit = @min(osk_text_limit(request.max_bytes), dst.len - 1);
    const text = valid_utf8_prefix(request.initial, limit);
