...
pub fn begin_text_input_session(input: input_api.EventSink, request: *const input_api.TextInputRequest) input_api.TextSessionError!void {
    ...
    const desc_len = std.unicode.utf8ToUtf16Le(&desc_buf, request.prompt) catch 0;
    if (desc_len < desc_buf.len) desc_buf[desc_len] = 0;

    const max_chars: usize = if (request.max_bytes) |m| @min(m, 256) else 256;
    var out_buf: [257]u16 = @splat(0);
    const out_slice = out_buf[0 .. max_chars + 1];

    var in_buf: [257]u16 = @splat(0);
    var in_len: usize = 0;
    if (request.initial.len > 0) {
        const cap: usize = in_buf.len - 1;
        const written = std.unicode.utf8ToUtf16Le(&in_buf, request.initial) catch 0;
    ...
    const status: input_api.TextInputResult = if (result == 0) .submitted else .cancelled;
    input.complete_text(utf8_buf[0..u_len], status);
}
...
fn diff_buttons(input: input_api.EventSink, now: ctrl.Data) void {
    const Pair = struct { field: []const u8, button: input_api.Button };
    const map = [_]Pair{
    ...
    inline for (map) |entry| {
        ...
        if (now_bit != prev_bit) {
            const edge: input_api.ButtonState = if (now_bit == 1) .pressed else .released;
            input.deliver_gamepad_button(entry.button, edge);
        }
    }
}

fn diff_axes(input: input_api.EventSink, now: ctrl.Data) void {
