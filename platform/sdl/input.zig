...
//! translates events into `EventSink.deliver_*` calls; polls gamepad state
...
// -- translation tables -------------------------------------------------------
//
// Device Key/Button/Axis values are historical GLFW codes; they are opaque
// constants, so they stay untouched and SDL enums are translated here.

const ButtonMapping = struct { sdl: sdl3.gamepad.Button, core: input_api.Button };
...
const AxisMapping = struct { sdl: sdl3.gamepad.Axis, core: input_api.Axis };
...
fn map_scancode(scancode: SdlScancode) ?input_api.Key {
...
fn convert_mods(mod: sdl3.keycode.KeyModifier) input_api.ModifierSet {
    var set: input_api.ModifierSet = .{};
...
fn handle_event(input: input_api.EventSink, event: sdl3.events.Event) void {
    switch (event) {
        ...
        .mouse_button_down, .mouse_button_up => |e| {
            const mb: input_api.MouseButton = switch (e.button) {
                .left => .Left,
                .right => .Right,
                .middle => .Middle,
...
fn pump_gamepad(input: input_api.EventSink) void {
